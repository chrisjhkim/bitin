...
<body>
					...
					<form action="#">
						<div class="btn-group-demo" data-toggle="buttons">
							<div class="btn-group btn-group-justified" id="modal-radio">
								...
								<label id="modal-label1" class="btn btn-default"
									onclick="modalClick_1();"> <input type="radio"
									name="options" id="modal-option1">출석
								</label> <label id="modal-label2" class="btn btn-default"
									onclick="modalClick_2();"> <input type="radio"
									name="options" id="modal-option2">지각
								</label> <label id="modal-label3" class="btn btn-default"
									onclick="modalClick_3();"> <input type="radio"
									name="options" id="modal-option3" checked="checked">결석
								</label> <label id="modal-label4" class="btn btn-default"
									onclick="modalClick_4();"> <input type="radio"
									name="options" id="modal-option4">병결
								</label>
							</div>
						</div>
						<div class="form-group">
							<label for="input-pwd">메모</label> <input class="form-control"
								type="password" id="input-pwd" name="userPassword"
								placeholder="한줄 메모 입력" />
						</div>
						<button class="btn btn-primary" data-dismiss="modal"
							onclick="modalModify();">수정</button>
			...
			<div class="card">
				...
				<div class="col-sm-5 actionBar">
					<div class="input-group form-group">
						...
						<div class="dtp-container fg-line">
							...
							<button class="btn btn-default form-control btn-icon-text"
								type="button" id="end-date" style="color:#5e5e5e;">
								종료 일자
							</button>
				...
				<div class="col-sm-2 actionBar">
					<button class="btn btn-success btn-block" type="button"
						onclick="modifyDateRange();">Apply</button>
				</div>
				<div class="table-responsive">
					<table id="data-table-basic" class="table table-striped"
						...
						<thead>
							<tr id="table-abc">
								<th data-column-id="name" data-type="text"
									data-identifier="true">이름</th>
							...
							<th data-column-id="attdYesOrInfoOrLateRate" data-visible="true">출석률(지각포함)</th> -->
								...
								<c:if test="${attdCounter>1}">
									<th data-column-id="fixed2" data-formatter="newtest2">${dataList[0].attdList[1].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>2}">
									<th data-column-id="fixed3" data-formatter="newtest3">${dataList[0].attdList[2].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>3}">
									<th data-column-id="fixed4" data-formatter="newtest4">${dataList[0].attdList[3].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>4}">
									<th data-column-id="fixed5" data-formatter="newtest5">${dataList[0].attdList[4].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>5}">
									<th data-column-id="fixed6" data-formatter="newtest6">${dataList[0].attdList[5].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>6}">
									<th data-column-id="fixed7" data-formatter="newtest7">${dataList[0].attdList[6].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>7}">
									<th data-column-id="fixed8" data-formatter="newtest8">${dataList[0].attdList[7].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>8}">
									<th data-column-id="fixed9" data-formatter="newtest9">${dataList[0].attdList[8].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>9}">
									<th data-column-id="fixed10" data-formatter="newtest10">${dataList[0].attdList[9].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>10}">
									<th data-column-id="fixed11" data-formatter="newtest11">${dataList[0].attdList[10].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>11}">
									<th data-column-id="fixed12" data-formatter="newtest12">${dataList[0].attdList[11].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>12}">
									<th data-column-id="fixed13" data-formatter="newtest13">${dataList[0].attdList[12].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>13}">
									<th data-column-id="fixed14" data-formatter="newtest14">${dataList[0].attdList[13].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>14}">
									<th data-column-id="fixed15" data-formatter="newtest15">${dataList[0].attdList[14].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>15}">
									<th data-column-id="fixed16" data-formatter="newtest16">${dataList[0].attdList[15].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>16}">
									<th data-column-id="fixed17" data-formatter="newtest17">${dataList[0].attdList[16].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>17}">
									<th data-column-id="fixed18" data-formatter="newtest18">${dataList[0].attdList[17].classDate }</th>
								</c:if>
								<c:if test="${attdCounter>18}">
									<th data-column-id="fixed19" data-formatter="newtest19">${dataList[0].attdList[18].classDate }</th>
								</c:if>
							</tr>
						</thead>
						<tbody>
							<c:forEach items='${dataList }' var='vo' varStatus='status'>
								<tr>
									...
									<c:if test="${attdCounter>10 }">
										<td>${vo.attdList[10].attdStatus }</td>
									</c:if>
									<c:if test="${attdCounter>11 }">
										<td>${vo.attdList[11].attdStatus }</td>
									</c:if>
									<c:if test="${attdCounter>12 }">
										<td>${vo.attdList[12].attdStatus }</td>
									</c:if>
									<c:if test="${attdCounter>13 }">
										<td>${vo.attdList[13].attdStatus }</td>
									</c:if>
									<c:if test="${attdCounter>14 }">
										<td>${vo.attdList[14].attdStatus }</td>
									</c:if>
									<c:if test="${attdCounter>15 }">
										<td>${vo.attdList[15].attdStatus }</td>
									</c:if>
									<c:if test="${attdCounter>16 }">
										<td>${vo.attdList[16].attdStatus }</td>
									</c:if>
									<c:if test="${attdCounter>17 }">
										<td>${vo.attdList[17].attdStatus }</td>
									</c:if>
									<c:if test="${attdCounter>18 }">
										<td>${vo.attdList[18].attdStatus }</td>
									</c:if>
	...
	<!-- 추가함 (준현)  -->
	<script
		src="/bitin/assets/vendors/bower_components/flot/jquery.flot.pie.js"></script>
	<script
		src="/bitin/assets/vendors/bower_components/flot.tooltip/js/jquery.flot.tooltip.min.js"></script>
	<script
...
>>>>>>> branch 'master' of https://github.com/chrisjhkim/bitin.git
								...
								console.log('gDate' + gDate + '// gUserNo :'
										+ gUserNo);
								console.log('tempDate ' + tempDate
										+ '// tempUserNo : ' + tempUserNo
										+ 'tempUserName' + tempUserName)
								console.log("length : " + tempDate.length)

								var newTag;
								if (tempDate.length > 6) {
									newTag = "<h4>" + tempUserName + "님의 "
											+ gDateMonth + "월 " + gDateDay
											+ "일 " + gDate.substring(7)
											+ "번째 출석을 수정합니다</h4>";
								} else {
									newTag = "<h4>" + gDateMonth + "월 "
											+ gDateDay + "일 " + tempUserName
											+ "의 출석을 수정합니다</h4>";
								}
								modal.find('#hoho').empty();
								modal.find('#hoho').prepend(newTag);
								...
								var tempStatus = $(
										'label[data-date=' + gDate
												...
												+ ']').attr('data-status');
								if (tempStatus == "no") {
									modal.find('#modal-label3').addClass(
											'btn-danger');

									modal.find('#modal-label1').removeClass(
											'btn-success');
									modal.find('#modal-label2').removeClass(
											'btn-warning');
									modal.find('#modal-label4').removeClass(
											'btn-info')

								} else if (tempStatus == "yes") {
									modal.find('#modal-label1').addClass(
											'btn-success');

									modal.find('#modal-label2').removeClass(
											'btn-warning');
									modal.find('#modal-label3').removeClass(
											'btn-danger');
									modal.find('#modal-label4').removeClass(
											'btn-info')
								} else if (tempStatus == "late") {
									modal.find('#modal-label2').addClass(
											'btn-warning');
									...
									modal.find('#modal-label4').addClass(
											'btn-info')
			...
			$("#data-table-basic").bootgrid({
				...
				formatters : {
					"newtest1" : function(column, row) {
						if (row.fixed1 == "yes") {
							return greenO(column, row, row.fixed1);
						} else if (row.fixed1 == "no") {
							return redX(column, row, row.fixed1);
						} else if (row.fixed1 == "late") {
							return orangeTriangle(column, row, row.fixed1);
						} else if (row.fixed1 == "info") {
							return blueInfo(column, row, row.fixed1);
						} else
							return "?";
					},
					"newtest2" : function(column, row) {
						if (row.fixed2 == "yes") {
							return greenO(column, row, row.fixed2);
						} else if (row.fixed2 == "no") {
							return redX(column, row, row.fixed2);
						} else if (row.fixed2 == "late") {
							return orangeTriangle(column, row, row.fixed2);
						} else if (row.fixed2 == "info") {
							return blueInfo(column, row, row.fixed2);
						} else
							return "?";
					},
					"newtest3" : function(column, row) {
						if (row.fixed3 == "yes") {
							return greenO(column, row, row.fixed3);
						} else if (row.fixed3 == "no") {
							return redX(column, row, row.fixed3);
						} else if (row.fixed3 == "late") {
							return orangeTriangle(column, row, row.fixed3);
						} else if (row.fixed3 == "info") {
							return blueInfo(column, row, row.fixed3);
						} else
							return "?";
					},
					"newtest4" : function(column, row) {
						if (row.fixed4 == "yes") {
							return greenO(column, row, row.fixed4);
						} else if (row.fixed4 == "no") {
							return redX(column, row, row.fixed4);
						} else if (row.fixed4 == "late") {
							return orangeTriangle(column, row, row.fixed4);
						} else if (row.fixed4 == "info") {
							return blueInfo(column, row, row.fixed4);
						} else
							return "?";
					},
					"newtest5" : function(column, row) {
						if (row.fixed5 == "yes") {
							return greenO(column, row, row.fixed5);
						} else if (row.fixed5 == "no") {
							return redX(column, row, row.fixed5);
						} else if (row.fixed5 == "late") {
							return orangeTriangle(column, row, row.fixed5);
						} else if (row.fixed5 == "info") {
							return blueInfo(column, row, row.fixed5);
						} else
							return "?";
					},
					"newtest6" : function(column, row) {
						if (row.fixed6 == "yes") {
							return greenO(column, row, row.fixed6);
						} else if (row.fixed6 == "no") {
							return redX(column, row, row.fixed6);
						} else if (row.fixed6 == "late") {
							return orangeTriangle(column, row, row.fixed6);
						} else if (row.fixed6 == "info") {
							return blueInfo(column, row, row.fixed6);
						} else
							return "?";
					},
					"newtest7" : function(column, row) {
						if (row.fixed7 == "yes") {
							return greenO(column, row, row.fixed7);
						} else if (row.fixed7 == "no") {
							return redX(column, row, row.fixed7);
						} else if (row.fixed7 == "late") {
							return orangeTriangle(column, row, row.fixed7);
						} else if (row.fixed7 == "info") {
							return blueInfo(column, row, row.fixed7);
						} else
							return "?";
					},
					"newtest8" : function(column, row) {
						if (row.fixed8 == "yes") {
							return greenO(column, row, row.fixed8);
						} else if (row.fixed8 == "no") {
							return redX(column, row, row.fixed8);
						} else if (row.fixed8 == "late") {
							return orangeTriangle(column, row, row.fixed8);
						} else if (row.fixed8 == "info") {
							return blueInfo(column, row, row.fixed8);
						} else
							return "?";
					},
					"newtest9" : function(column, row) {
						if (row.fixed9 == "yes") {
							return greenO(column, row, row.fixed9);
						} else if (row.fixed9 == "no") {
							return redX(column, row, row.fixed9);
						} else if (row.fixed9 == "late") {
							return orangeTriangle(column, row, row.fixed9);
						} else if (row.fixed9 == "info") {
							return blueInfo(column, row, row.fixed9);
						} else
							return "?";
					},
					"newtest10" : function(column, row) {
						if (row.fixed10 == "yes") {
							return greenO(column, row, row.fixed10);
						} else if (row.fixed10 == "no") {
							return redX(column, row, row.fixed10);
						} else if (row.fixed10 == "late") {
							return orangeTriangle(column, row, row.fixed10);
						} else if (row.fixed10 == "info") {
							return blueInfo(column, row, row.fixed10);
						} else
							return "?";
					},
					"newtest11" : function(column, row) {
						if (row.fixed11 == "yes") {
							return greenO(column, row, row.fixed11);
						} else if (row.fixed11 == "no") {
							return redX(column, row, row.fixed11);
						} else if (row.fixed11 == "late") {
							return orangeTriangle(column, row, row.fixed11);
						} else if (row.fixed11 == "info") {
							return blueInfo(column, row, row.fixed11);
						} else
							return "?";
					},
					"newtest12" : function(column, row) {
						if (row.fixed12 == "yes") {
							return greenO(column, row, row.fixed12);
						} else if (row.fixed12 == "no") {
							return redX(column, row, row.fixed12);
						} else if (row.fixed12 == "late") {
							return orangeTriangle(column, row, row.fixed12);
						} else if (row.fixed12 == "info") {
							return blueInfo(column, row, row.fixed12);
						} else
							return "?";
					},
					"newtest13" : function(column, row) {
						if (row.fixed13 == "yes") {
							return greenO(column, row, row.fixed13);
						} else if (row.fixed13 == "no") {
							return redX(column, row, row.fixed13);
						} else if (row.fixed13 == "late") {
							return orangeTriangle(column, row, row.fixed13);
						} else if (row.fixed13 == "info") {
							return blueInfo(column, row, row.fixed13);
						} else
							return "?";
					},
					"newtest14" : function(column, row) {
						if (row.fixed14 == "yes") {
							return greenO(column, row, row.fixed14);
						} else if (row.fixed14 == "no") {
							return redX(column, row, row.fixed14);
						} else if (row.fixed14 == "late") {
							return orangeTriangle(column, row, row.fixed14);
						} else if (row.fixed14 == "info") {
							return blueInfo(column, row, row.fixed14);
						} else
							return "?";
					},
					"newtest15" : function(column, row) {
						if (row.fixed15 == "yes") {
							return greenO(column, row, row.fixed15);
						} else if (row.fixed15 == "no") {
							return redX(column, row, row.fixed15);
						} else if (row.fixed15 == "late") {
							return orangeTriangle(column, row, row.fixed15);
						} else if (row.fixed15 == "info") {
							return blueInfo(column, row, row.fixed15);
						} else
							return "?";
					},
					"newtest16" : function(column, row) {
						if (row.fixed16 == "yes") {
							return greenO(column, row, row.fixed16);
						} else if (row.fixed16 == "no") {
							return redX(column, row, row.fixed16);
						} else if (row.fixed16 == "late") {
							return orangeTriangle(column, row, row.fixed16);
						} else if (row.fixed16 == "info") {
							return blueInfo(column, row, row.fixed16);
						} else
							return "?";
					},
					"newtest17" : function(column, row) {
						if (row.fixed17 == "yes") {
							return greenO(column, row, row.fixed17);
						} else if (row.fixed17 == "no") {
							return redX(column, row, row.fixed17);
						} else if (row.fixed17 == "late") {
							return orangeTriangle(column, row, row.fixed17);
						} else if (row.fixed17 == "info") {
							return blueInfo(column, row, row.fixed17);
						} else
							return "?";
					},
					"newtest18" : function(column, row) {
						if (row.fixed18 == "yes") {
							return greenO(column, row, row.fixed18);
						} else if (row.fixed18 == "no") {
							return redX(column, row, row.fixed18);
						} else if (row.fixed18 == "late") {
							return orangeTriangle(column, row, row.fixed18);
						} else if (row.fixed18 == "info") {
							return blueInfo(column, row, row.fixed18);
						} else
							return "?";
					},
					"newtest19" : function(column, row) {
						if (row.fixed19 == "yes") {
							return greenO(column, row, row.fixed19);
						} else if (row.fixed19 == "no") {
							return redX(column, row, row.fixed19);
						} else if (row.fixed19 == "late") {
							return orangeTriangle(column, row, row.fixed19);
						} else if (row.fixed19 == "info") {
							return blueInfo(column, row, row.fixed19);
						} else
							return "?";
					},
	...
	<script type="text/javascript">
		...
		function modifyDateRange() {
			...
			if (dateRangeStart > dateRangeEnd) {
				...
				targetUrl = "/bitin/chris/main?from=" + dateRangeStart + "&to="
						+ dateRangeEnd;
	...
	<script type="text/javascript">
		function modalModify() {
			...
			if (modal.find('#modal-label1').hasClass('btn-success')) {
				//모양 변경
				$(
						'label[data-date=' + gDate + '][data-userno=' + gUserNo
								+ '] i').addClass('zmdi-circle-o')
				$(
						'label[data-date=' + gDate + '][data-userno=' + gUserNo
								+ '] i').removeClass('zmdi-close')
				$(
						'label[data-date=' + gDate + '][data-userno=' + gUserNo
								+ '] i').removeClass('zmdi-triangle-up')
				$(
						'label[data-date=' + gDate + '][data-userno=' + gUserNo
								+ '] i').removeClass('zmdi-alert-circle-o')
				//색 변경
				$('label[data-date=' + gDate + '][data-userno=' + gUserNo + ']')
				...
				$(
						'label[data-date=' + gDate + '][data-userno=' + gUserNo
								+ '] i').parent().removeClass('text-info');
				...
				$(
						'label[data-date=' + gDate + '][data-userno=' + gUserNo
								+ '] i').removeClass('zmdi-circle-o')
				$(
						'label[data-date=' + gDate + '][data-userno=' + gUserNo
								+ '] i').removeClass('zmdi-close')
				$(
						'label[data-date=' + gDate + '][data-userno=' + gUserNo
								+ '] i').addClass('zmdi-triangle-up')
				$(
						'label[data-date=' + gDate + '][data-userno=' + gUserNo
								+ '] i').removeClass('zmdi-alert-circle-o')
				//색 변경
				...
				$(
						'label[data-date=' + gDate + '][data-userno=' + gUserNo
								+ '] i').addClass('zmdi-alert-circle-o')
				//색 변경
				$('label[data-date=' + gDate + '][data-userno=' + gUserNo + ']')
						.parent().removeClass('has-success');
				$('label[data-date=' + gDate + '][data-userno=' + gUserNo + ']')
			...
			console.log(gDate + '일  ' + gUserNo + '번 유저의 수업상태를 ' + gStatus
					+ "로 바꾼다.")

			var parsedDate = gDate.substring(0, 2) + "M"
					+ gDate.substring(3, 5) + "D";
			if (gDate.length > 6)
				parsedDate += gDate.substring(7);
			...
			$.ajax({
				...
				type : "get",
				dataType : "json",
				data : "classDate=" + parsedDate + "&userNo=" + gUserNo
						+ "&attdStatus=" + gStatus + "&firstAttdNo="
						+ firstAttdNo,
	...
	<script type="text/javascript">
		function valueGetter(userNo, date) {
			...
			console.log('tempDate ' + tempDate + '// tempUserNo : '
					+ tempUserNo + ' // tempUserName' + tempUserName)
			gDate = date;
			gUserNo = userNo;
			$('div.modal').modal();
		}
