...
//    , id          = Math.floor(Math.random() * 100000) //Create semi-unique ID incase user doesn't selet one
    , id          = 500 //Create semi-unique ID incase user doesn't selet one
    , x           = d3.scale.linear()
    , y           = d3.scale.linear()
        ...
        var vertices = d3.merge(data.map(function(group, groupIndex) {
            return group.values
              .map(function(point, pointIndex) {
                ...
                return [x(getX(point,pointIndex)) * (120 / 1e12 + 1)  , y(getY(point,pointIndex)) * (150 / 1e12 + 1), groupIndex, pointIndex, point]; //temp hack to add noise untill I think of a better way so there are no duplicates
